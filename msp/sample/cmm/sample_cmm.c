...
static int ax_mem_cmm_test_015(void)
{
    ...
    AX_U32 TEST_TIME = 100;
    AX_U32 fail_count = 0;
    AX_CMM_STATUS_T stCmmStatus = { 0 };

    printf("\nax_mem_cmm_test_015 begin\n");

    nRet = AX_SYS_MemQueryStatus(&stCmmStatus);
    if (nRet) {
        return -1;
    }

    for (i = 0; i < stCmmStatus.Partition.PartitionCnt; i++) {
        if (!strcmp((char *)stCmmStatus.Partition.PartitionInfo[i].Name, "anonymous"))
            break;
    }

    PhyAddr = stCmmStatus.Partition.PartitionInfo[i].PhysAddr + stCmmStatus.Partition.PartitionInfo[i].SizeKB * 1024 - BlockSize * 2;

    printf("BaseAddr=0x%llx, Size=%d, PhyAddr=0x%llx\n",
           stCmmStatus.Partition.PartitionInfo[i].PhysAddr,
           stCmmStatus.Partition.PartitionInfo[i].SizeKB,
           PhyAddr);
