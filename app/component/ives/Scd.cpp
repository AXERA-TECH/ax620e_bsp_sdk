...
AX_BOOL CSCD::Startup(AX_S32 nSnsId, AX_U32 nWidth, AX_U32 nHeight) {
    if (-1 == nSnsId) {
...
AX_VOID CSCD::Cleanup(AX_VOID) {
    ...
    if (-1 != m_chn) {
        LOG_MM_I(SCD, "AX_IVES_SCD_DestoryChn[%d]", m_chn);
...
AX_VOID CSCD::UpdateResolution(AX_U32 w, AX_U32 h) {
    Cleanup();
...
AX_VOID CSCD::SetConfig(const AX_APP_ALGO_SCENE_CHANGE_PARAM_T &stConfig, AX_BOOL bUpdateParam) {
    ...
    if (bUpdateParam) {
        SET_ALGO_SCD_PARAM(m_nSnsID, stConfig);
    }
}
