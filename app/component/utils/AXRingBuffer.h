...
public:
    CAXRingBuffer(AX_U32 nElementBuffSize, AX_U32 nElementCount, const char* pszName = nullptr) {
        ...
        for (AX_U32 i = 0; i < m_nElementCount; i++) {
            m_pRing[i].pBuf = (AX_U8 *)AX_MALLOC(m_nElementBuffSize);
    ...
    AX_BOOL Put(CAXRingElement& element) {
        ...
        if ((m_nTail - m_nHeader) == m_nElementCount) {
            // is full
            LOG_MM_W(AXRING, "[%s] is full, tail=%llu, head=%llu", m_szName, m_nTail, m_nHeader);
            if (element.bIFrame) {
                ...
                if (nRefCount == 0) {
                    m_nTail--;
                    m_bHasLost = AX_FALSE;
                    LOG_MM_W(AXRING, "[%s] ---, replace last one with i frame for is full", m_szName);
                } else {
                    m_bHasLost = AX_TRUE;
                    LOG_MM_W(AXRING, "[%s] ---, drop one i frame for is full", m_szName);
                    return AX_FALSE;
                }

            } else {
                m_bHasLost = AX_TRUE;  // mark to lost all behind P Frame
                LOG_MM_W(AXRING, "[%s] ---, drop one p frame for is full", m_szName);
                return AX_FALSE;
            }
            ...
            if (element.bIFrame) {
                ...
                if (m_bHasLost) {
                    // drop this P Frame
                    LOG_MM_W(AXRING, "[%s] ---, drop one p frame for lost yet", m_szName);
                    return AX_FALSE;
                }
            }
        }
        ...
        if (nRefCount != 0) {
            m_bHasLost = AX_TRUE;
            LOG_MM_W(AXRING, "[%s] ---, drop one frame for refcount is none zero, nIndex=%llu, nRefCount=%d", m_szName, nIndex, nRefCount);
            return AX_FALSE;
        }
