...
)
{
    ...
    for (int i = 0; i < pCtx->tStatus.blkRowNum ; i++) {
        for(int j = 0; j < pCtx->tStatus.blkColNum; j++) {
            ...
            int idx = i * pCtx->tStatus.blkColNum + j;
        ...
        // Calculate the R, G, B, Y Values of Each AWB Statistics Block.
            if(pAwbStat->tAwbGridStats.tAwbGridStats[idx].nUnSatGridNum[0] == 0) {

                pCtx->tStatus.blkRg[idx] = 0.0f;
                pCtx->tStatus.blkBg[idx] = 0.0f;
                pCtx->tStatus.blkY[idx] = 0.0f;

                continue;
            }

            AX_U32 gridNum = pAwbStat->tAwbGridStats.tAwbGridStats[idx].nUnSatGridNum[0];
            rSum  =  pAwbStat->tAwbGridStats.tAwbGridStats[idx].nUnSatGridSum[0][0];
            bSum  =  pAwbStat->tAwbGridStats.tAwbGridStats[idx].nUnSatGridSum[0][1];
            grSum =  pAwbStat->tAwbGridStats.tAwbGridStats[idx].nUnSatGridSum[0][2];
            gbSum =  pAwbStat->tAwbGridStats.tAwbGridStats[idx].nUnSatGridSum[0][3];
