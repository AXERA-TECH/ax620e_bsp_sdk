    ...
    .stVoLayer = {
        {
           ...
           /* .bindVoDev = {SAMPLE_VO_DEV_MAX, SAMPLE_VO_DEV_MAX, SAMPLE_VO_DEV_MAX}, */
            ...
            .u64KeepChnPrevFrameBitmap0 = 0xffffUL,
...
AX_VOID SAMPLE_VoPrintReso(AX_VOID)
{
    ...
    printf("\n  VO display interface resolution and VO device\n");
...
AX_S32 SAMPLE_VoInit(SAMPLE_VO_INIT_PARAM_S *pInitParam, AX_U16 nChnNum)
{
    ...
    nWidth = pInitParam->nWidth;
    nHeight = pInitParam->nHeight;
    s32Ret = AX_VO_Init();
    if (s32Ret) {
    ...
    for (i = 0; i < pstVoConf->u32LayerNr; i++) {
        ...
        pstVoDevConf->enVoIntfType = pInitParam->enVoIntfType;
        pstVoDevConf->enIntfSync = pInitParam->enIntfSync;
        ...
        case 1:
            ...
            break;
        case 2:
            pstVoLayer->enVoMode = VO_MODE_2MUX;
            pstVoLayer->u32ChnNr = 2;
            break;
        case 3:
        case 4:
            pstVoLayer->enVoMode = VO_MODE_4MUX;
            pstVoLayer->u32ChnNr = 4;
            break;
        case 5:
        case 6:
        case 7:
        case 8:
            pstVoLayer->enVoMode = VO_MODE_8MUX;
            pstVoLayer->u32ChnNr = 8;
            break;
        case 9:
            pstVoLayer->enVoMode = VO_MODE_9MUX;
            pstVoLayer->u32ChnNr = 9;
            break;
        default:
            pstVoLayer->enVoMode = VO_MODE_16MUX;
            pstVoLayer->u32ChnNr = 16;
            break;
        }
        ...
        if (pstVoLayer->stVoLayerAttr.stCompressInfo.enCompressMode == AX_COMPRESS_MODE_NONE)
            u64BlkSize = (AX_U64)ALIGN_UP(pstVoLayerAttr->stImageSize.u32Width, 16) * ALIGN_UP(pstVoLayerAttr->stImageSize.u32Height, 2) * 3 / 2;
        else
            u64BlkSize = (AX_U64)ALIGN_UP(pstVoLayerAttr->stImageSize.u32Width, 128) * ALIGN_UP(pstVoLayerAttr->stImageSize.u32Height, 2) * 3 / 2;
        s32Ret = SAMPLE_VO_CREATE_POOL(5, u64BlkSize, 512, &pstVoLayerAttr->u32PoolId);
        if (s32Ret) {
...
{
    AX_U32 u32Width = 800;
    AX_U32 u32Height = 480;

    if (!pWidth || !pHeight) {
        SAMPLE_PRT("inv prm pWidth %p pHeight %p\n", pWidth, pHeight);
        return -EINVAL;
    }

    switch(enVoSync) {
    case AX_VO_OUTPUT_800x480_60:
        u32Width = 800;
        u32Height = 480;
        break;
    case AX_VO_OUTPUT_800x600_60:
        u32Width = 800;
        u32Height = 600;
        break;
    ...
    case AX_VO_OUTPUT_1080x1920_60:
        ...
        break;
    default:
        u32Width = stVoConf.stVoLayer[0].stVoLayerAttr.stImageSize.u32Width;
        u32Height = stVoConf.stVoLayer[0].stVoLayerAttr.stImageSize.u32Height;
        break;
    }
