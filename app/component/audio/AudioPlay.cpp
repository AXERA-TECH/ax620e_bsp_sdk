...
AX_BOOL CAudioPlay::Init(const AUDIO_PLAY_DEV_ATTR_PTR pstPlayDevAttr, AX_U32 nPipeNum, const ADEC_CONFIG_PTR pstPipeAttr) {
    ...
    if (!pstPlayDevAttr || (0 == nPipeNum) || !pstPipeAttr) {
    ...
    // STEP1: set link
    if (nPipeNum > 1 || pstPipeAttr[0].bLink != m_bLink) {
        m_bLink = AX_FALSE;
    }
...
AX_BOOL CAudioPlay::Play(AX_PAYLOAD_TYPE_E eType, const AX_U8 *pData, AX_U32 nDataSize, AX_U64 u64SeqNum, AX_U32 nChannel /* = 0*/) {
    AX_BOOL bRet = AX_FALSE;
...
AX_BOOL CAudioPlay::PlayFile(AX_PAYLOAD_TYPE_E eType, const std::string &strFileName, AX_S32 nLoop, AUDIO_FILE_PLAY_CALLBACK callback,
                             AX_VOID *pUserData, AX_U32 nChannel /* = AUDIO_FILE_PLAY_DEFAULT_CHANNEL*/) {
    ...
    return pAudioFilePlay->Play(eType, strFileName, nLoop, callback, pUserData);
}
