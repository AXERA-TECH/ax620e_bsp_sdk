...
AX_BOOL CIspAlgoWrapper::RegisterAlgoToSensor(AX_SENSOR_REGISTER_FUNC_T *pSnsObj, AX_U8 nPipe) {
    ...
    if (m_tAlg.bActiveAwb) {
        ...
        if (!m_tAlg.bUserAwb) {
            /* Register the sensor driven interface TO the AE library */
            ret = AX_ISP_ALG_AwbRegisterSensor(nPipe, pSnsObj);
            if (0 != ret) {
                LOG_M_E(ALGO_WRAPPER, "AX_ISP_ALG_AwbRegisterSensor(pipe: %d) fail, ret = 0x%08X", nPipe, ret);
                return AX_FALSE;
            }
        }
...
AX_BOOL CIspAlgoWrapper::UnRegisterAlgoFromSensor(AX_U8 nPipe) {
    ...
    if (m_tAlg.bActiveAwb) {
        if (!m_tAlg.bUserAwb) {
            ret = AX_ISP_ALG_AwbUnRegisterSensor(nPipe);
            if (0 != ret) {
                LOG_M_E(ALGO_WRAPPER, "AX_ISP_ALG_AwbUnRegisterSensor(pipe: %d) fail, ret = 0x%08X", nPipe, ret);
                return AX_FALSE;
            }
        }
