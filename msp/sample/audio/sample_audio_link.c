...
static void *AencRecvThread(void *arg)
{
    ...
    while (1) {
        ...
        if (gWriteFrames && pstStream.u32Len && (aencArgs->payloadType == PT_AAC) && (gTransType == AX_AAC_TRANS_TYPE_RAW)) {
...
static int AudioInput()
{
    ...
    if (gDbDetection) {
        ...
        stAedAttr.bDbDetection = gDbDetection;
...
static int AudioOutput()
{
    ...
    char mix_file_name[FILE_NAME_SIZE];
    FILE *mix_file = NULL;
    if (gMix) {
        if (gMixFile) {
            strncpy(mix_file_name, gMixFile, FILE_NAME_SIZE-1);
        } else {
            snprintf(mix_file_name, FILE_NAME_SIZE, "audio_mix.%s", gIsWave ? "wav" : "raw");
        }
        mix_file = fopen(mix_file_name, "rb");
        if (mix_file == NULL) {
            printf("failed to open '%s' for reading\n", mix_file_name);
            goto FCLOSE_INPUT_FILE;
        }
    }
    ...
    if (gIsWave) {
        ...
        if (gMix) {
            uint16_t mix_bits_per_sample;
            if (ParseWaveHeader(mix_file_name, mix_file, &mix_num_channels, &mix_sample_rate, &mix_bits_per_sample)) {
                fprintf(stderr, "ParseWaveHeader '%s' error\n", mix_file_name);
                goto FCLOSE_MIX_FILE;
            }
            if (BitsToFormat(mix_bits_per_sample, &mix_format)) {
                goto FCLOSE_MIX_FILE;
            }

            if ((mix_num_channels != num_channels) ||
                (mix_sample_rate != sample_rate) ||
                (mix_format != format)) {
                printf("mix_file and input_file format not match\n");
                goto FCLOSE_MIX_FILE;
            }
        }
    }
    ...
    if (gHpfCfg.bEnable) {
        ...
        stHpfAttr.s32GainDb = gHpfCfg.s32GainDb;
        stHpfAttr.s32Samplerate = gRate;
        ret = AX_ACODEC_TxHpfSetAttr(card, &stHpfAttr);
        if(ret){
    ...
    if (gLpfCfg.bEnable) {
        ...
        stLpfAttr.s32GainDb = gLpfCfg.s32GainDb;
        stLpfAttr.s32Samplerate = gRate;
        ret = AX_ACODEC_TxLpfSetAttr(card, &stLpfAttr);
        if(ret){
    ...
    if (gEqCfg.bEnable) {
        ...
        memcpy(&stEqAttr.s32GainDb, &gEqCfg.s32GainDb, sizeof(stEqAttr.s32GainDb));
        stEqAttr.s32Samplerate = gRate;
        ret = AX_ACODEC_TxEqSetAttr(card, &stEqAttr);
        if(ret){
    ...
    int sendCount = 0;
    AX_S32 loopNumber = 0;
    AX_AUDIO_FRAME_T stMixFrmInfo;
    memset(&stMixFrmInfo, 0, sizeof(stMixFrmInfo));
    stMixFrmInfo.enBitwidth = mix_format;
    stMixFrmInfo.enSoundmode = (mix_num_channels == 1 ? AX_AUDIO_SOUND_MODE_MONO : AX_AUDIO_SOUND_MODE_STEREO);
    int mix_num_read;
    AX_S32 readMixFrm = gMix;
    AX_F32 f32MixWeight = readMixFrm ? 0.5 : 0.f;
    do {
        ...
        if (readMixFrm) {
            stMixFrmInfo.u32BlkId = AX_POOL_GetBlock(PoolId, BlkSize, NULL);
            stMixFrmInfo.u64VirAddr = AX_POOL_GetBlockVirAddr(stMixFrmInfo.u32BlkId);
            mix_num_read = fread(stMixFrmInfo.u64VirAddr, 1, BlkSize, mix_file);
            if (mix_num_read <= 0) {
                AX_POOL_ReleaseBlock(stMixFrmInfo.u32BlkId);
                readMixFrm = 0;
            }
        }
        if (num_read > 0) {
            ...
            if (gSimDrop) {
                ...
                if (gGetNoise) {
                    ...
                    if (readMixFrm || f32MixWeight > 0.f) {
                        stMixFrmInfo.u32Len = readMixFrm ? mix_num_read : 0;
                        ret = AX_AO_SendFrame(card,device,&stFrmInfo,&stMixFrmInfo,f32MixWeight,-1);
                        if (!readMixFrm) {
                            f32MixWeight -= 0.01;
                        }
                    } else {
...
static int AudioEncodeLink()
{
    ...
    AX_POOL_CONFIG_T stPoolConfig;
    stPoolConfig.MetaSize = 8192;
    stPoolConfig.BlkSize = 8192;  /*FAAC, double mic need 8192 (1024*4*2rx) */
    stPoolConfig.BlkCnt = 33;
    stPoolConfig.IsMergeMode = AX_FALSE;
    stPoolConfig.CacheMode = AX_POOL_CACHE_MODE_NONCACHE;
    strcpy((char *)stPoolConfig.PartitionName, "anonymous");
...
static int AudioDecodeLink()
{
   ...
   case PT_G723:
    ...
    if (gInstant) {
        ...
        while (1) {
            ret = AX_AO_QueryDevStat(card, device, &stStatus);
            if (stStatus.u32DevBusyNum == 0 && stStatus.longPcmBufDelay <= 0) {
                break;
            }
            usleep(10*1000);
        }
...
DETACHPOOL_ADEC:
    ret = AX_ADEC_DetachPool(adChn);
    if (ret) {
        printf("AX_ADEC_DetachPool failed!ret = %x \n",ret);
    }
DESTROY_ADEC:
