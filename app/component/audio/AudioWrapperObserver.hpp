...
public:
    AX_BOOL OnRecvData(OBS_TARGET_TYPE_E eTarget, AX_U32 nGrp, AX_U32 nChn, AX_VOID* pData) override {
        ...
        if (E_OBS_TARGET_TYPE_AIRAW == eTarget) {
            ...
            AX_APP_AUDIO_DETECTRESULT_CALLBACK callback = (AX_APP_AUDIO_DETECTRESULT_CALLBACK)m_pSink;
            AX_APP_AUDIO_DETECT_RESULT_T stResult;
            AED_ALGO_RESULT_PTR pstResult = (AED_ALGO_RESULT_PTR)pData;

            memset(&stResult, 0x00, sizeof(stResult));
            stResult.fDb = pstResult->fDb;
            stResult.pUserData = m_pUserData;
            stResult.pPrivateData = nullptr;
