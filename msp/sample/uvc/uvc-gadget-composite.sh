...
CHIP=$(cat /proc/ax_proc/chip_type)
if [ "$CHIP" == "AX620Q_CHIP" ]; then
   CONFIGFS="/tmp/configfs"
fi
echo "config path: $CONFIGFS"
...
usb_acm_dev_controller=usb_acm
acm_UDC=${CONFIGFS}/usb_gadget/${usb_acm_dev_controller}/UDC

if [ $# -ne 5 ]; then
    echo "usage:"
    echo "$0 start|stop usb2 1|2 0|1 0|1"
    echo "param 1: command start or stop"
    echo "param 2: usb2 is supported"
    echo "param 3: uvc device count, support 2 devices at maximum"
    echo "param 4: 0: os04a10 sensor, 1: dummy sensor, 2: sc450ai sensor"
...
# 0: os04a10 sensor, 1: dummy sensor
sensor_type=${4}
...
if [ "usb2" != ${test_usb} ]; then
    echo "usb2 is supported"
    exit 1
fi
...
if [ 2 -lt ${sensor_type} -o 0 -gt ${sensor_type} ]; then
    echo "sensor_type is out of range, 0,1,2 are supported"
    exit 1
fi
...
create_uvc() {
    ...
    if [ 0 -eq ${sensor_type} -o 2 -eq ${sensor_type} ]; then
        echo "	os04a10/sc450ai sensor is selected."
        # support os04a10 sensor resolution
        # the max resolution is 2688x1520
        ...
        create_frame $FUNCTION 2688 1520 mjpeg m
        ...
        create_frame $FUNCTION 2688 1520 framebased fb
        ...
        create_frame $FUNCTION 2688 1520 framebased fb1
...
EOF
    ...
    if [ "$KERNEL_VERSION" == "5.15.73" ]; then
    ...
    if [ "$KERNEL_VERSION" == "5.15.73" ]; then
        ...
        echo "	$FUNCTION: maxpacketsize:$MAXPACKETSIZE, mult:$mult , burst:$burst, interval:$interval, stream_bulk:$bulk"
    fi
