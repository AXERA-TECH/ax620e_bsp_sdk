...
AX_BOOL COD::Startup(AX_S32 nSnsID, AX_U32 nFrameRate, AX_U32 nWidth, AX_U32 nHeight) {
    ...
    LOG_MM_I(OD, "+++nSnsID:%d, nFrameRate:%d, nWidth:%d, nHeight:%d", nSnsID, nFrameRate, nWidth, nHeight);
    ...
    if (!APP_SENSOR_CONFIG(nSnsID, tSensorCfg)) {
        LOG_M_E(OD, "Failed to get sensor config %d", nSnsID);
        return AX_FALSE;
    }
        ...
        if (m) {
            if (IS_ODCHN_CREATE(m->stAttr.odChn)) {
                ...
                if (0 != ret) {
                    ...
                    LOG_MM_I(OD, "AX_IVES_OD_DestoryChn[%d]", m->stAttr.odChn);
                }
            }
...
AX_S32 COD::AddArea(AX_U32 x, AX_U32 y, AX_U32 w, AX_U32 h, AX_U32 nWidth, AX_U32 nHeight, AX_S32 nSnsID) {
    ...
    OD_AREA_ATTR_T *pArea = (OD_AREA_ATTR_T *)AX_MALLOC(sizeof(OD_AREA_ATTR_T));
    if (!pArea) {
...
const std::vector<AX_U8> &COD::ProcessFrame(const AX_U32 nSnsID, const CAXFrame *pFrame) {
    ...
    if (0 == AX_ISP_IQ_GetAeStatus(m_nPipeID, &tAE)) {
        ...
        stImg.u32Lux = 0;
        LOG_M_E(OD, "AX_ISP_IQ_GetAeStatus(pipe: %d) fail", m_nPipeID);
    }
...
AX_VOID COD::UpdateResolution(AX_U32 w, AX_U32 h) {
    LOG_MM_I(OD, "+++ w:%d, h:%d", w, h);
    Cleanup();
    Startup(m_nSnsID, (AX_U32)m_nFrameRate, w, h);
    AddArea();
    LOG_MM_I(OD, "---");
}

AX_VOID COD::AddArea() {
    const AX_U32 nAreaW = m_odImgW / 2;
    const AX_U32 nAreaH = m_odImgH / 2;

    const AX_U32 nX = m_odImgW / 2 - nAreaW / 2;
    const AX_U32 nY = m_odImgH / 2 - nAreaH / 2;

    AddArea(nX, nY, nAreaW, nAreaH, m_odImgW, m_odImgH, m_nSnsID);
}
...
AX_VOID COD::SetConfig(const AX_APP_ALGO_OCCLUSION_PARAM_T &stConfig, AX_BOOL bUpdateParam) {
    if (stConfig.bEnable != m_stConfig.bEnable) {
        ...
        for (AX_U32 i = 0; i < nAreaCount; ++i) {
            OD_AREA_ATTR_T *m = m_vecAreas[i];
            m->stAttr.u8ThrdY = (AX_U8)stConfig.fThreshold;
            m->stAttr.u8ConfidenceY = (AX_U8)stConfig.fConfidence;
            m->stAttr.u32LuxThrd = (AX_U32)stConfig.fLuxThreshold;
            m->stAttr.u32LuxDiff = (AX_U32)stConfig.fLuxConfidence;
        }
    }
    ...
    if (bUpdateParam) {
        SET_ALGO_OD_PARAM(m_nSnsID, stConfig);
    }
}
