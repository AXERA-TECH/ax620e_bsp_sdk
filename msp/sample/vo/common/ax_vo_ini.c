                                 ...
                                 AX_VO_INI_SECTION_E type, AX_VO_INI_KEY_E key)
{
    ...
    case AX_VO_INI_K_C_FRAME_FMT:
        size = sizeof(vo_img_fmt_str) / sizeof(vo_img_fmt_str[0]);
        if (type == AX_VO_INI_S_LAYER_DISPLAY) {
            u32ValNr = stSplitStr.u32StrNr;
            pVoConfig = (SAMPLE_VO_CONFIG_S *)pConfig;
            for (i = 0; i < u32ValNr; i++) {
                for (index = 0; index < size; index++) {
                    if (!strcmp(stSplitStr.pStr[i], vo_img_fmt_str[index].name)) {
                        pLayerConf = &pVoConfig->stVoLayer[i];
                        pLayerConf->enChnFrmFmt = vo_img_fmt_str[index].fmt;
                    }
                }
            }
        } else {
            pLayerConf = (SAMPLE_VO_LAYER_CONFIG_S *)pConfig;
                for (index = 0; index < size; index++) {
                    if (!strcmp(stSplitStr.pStr[0], vo_img_fmt_str[index].name)) {
                        pLayerConf->enChnFrmFmt = vo_img_fmt_str[index].fmt;
                    }
                }
        }
    ...
    case AX_VO_INI_K_L_CHNS:
        if (type == AX_VO_INI_S_LAYER_DISPLAY) {
            ...
            for (i = 0; i < u32ValNr; i++) {
                pLayerConf = &pVoConfig->stVoLayer[i];
                pLayerConf->u32ChnNr = strtoul(stSplitStr.pStr[i], NULL, 0);
            }
        } else {
            pLayerConf = (SAMPLE_VO_LAYER_CONFIG_S *)pConfig;
            pLayerConf->u32ChnNr = strtoul(stSplitStr.pStr[0], NULL, 0);
        }
        break;
                ...
                if (!strcmp(stSplitStr.pStr[i], vo_fbcdc_mode_str[mode])) {
                    if (type == AX_VO_INI_S_LAYER_DISPLAY) {
                        ...
                        if (key == AX_VO_INI_K_C_FBDC_MODE) {
                            ...
                            pVoConfig->stVoLayer[i].stVoLayerAttr.stCompressInfo.enCompressMode = mode;
                        }
                    } else {
                        pLayerConf = (SAMPLE_VO_LAYER_CONFIG_S *)pConfig;
        ...
        if (type == AX_VO_INI_S_LAYER_DISPLAY) {
            ...
            for (i = 0; i < u32ValNr; i++) {
                ...
                if (key == AX_VO_INI_K_C_FBDC_LEVEL) {
                    ...
                    pLayerConf->stVoLayerAttr.stCompressInfo.u32CompressLevel = strtoul(stSplitStr.pStr[i], NULL, 0);
                }
            }
        } else {
            pLayerConf = (SAMPLE_VO_LAYER_CONFIG_S *)pConfig;
    ...
    case AX_VO_INI_K_L_BATCH_PROCESS:
        if (type == AX_VO_INI_S_LAYER_DISPLAY) {
            pVoConfig = (SAMPLE_VO_CONFIG_S *)pConfig;
            u32ValNr = stSplitStr.u32StrNr;
            for (i = 0; i < u32ValNr; i++) {
                pLayerConf = &pVoConfig->stVoLayer[i];
                pLayerConf->s32BatchProcFlag = strtoul(stSplitStr.pStr[i], NULL, 0) + 1;
            }
        } else {
            pLayerConf = (SAMPLE_VO_LAYER_CONFIG_S *)pConfig;
            pLayerConf->s32BatchProcFlag = strtoul(stSplitStr.pStr[0], NULL, 0) + 1;
        }
        break;

     case AX_VO_INI_K_L_BLEND_MODE:
        if (type == AX_VO_INI_S_LAYER_DISPLAY) {
            pVoConfig = (SAMPLE_VO_CONFIG_S *)pConfig;
            u32ValNr = stSplitStr.u32StrNr;
            for (i = 0; i < u32ValNr; i++) {
                pLayerConf = &pVoConfig->stVoLayer[i];
                pLayerConf->stVoLayerAttr.enBlendMode = strtoul(stSplitStr.pStr[i], NULL, 0);
            }
        } else {
            pLayerConf = (SAMPLE_VO_LAYER_CONFIG_S *)pConfig;
            pLayerConf->stVoLayerAttr.enBlendMode = strtoul(stSplitStr.pStr[0], NULL, 0);
        }
        break;
