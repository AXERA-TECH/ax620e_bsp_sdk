...
static AX_BOOL IsAudioSupport(AX_PAYLOAD_TYPE_E eType) {
    AX_BOOL bRet = (AX_BOOL)((PT_G711A == eType) || (PT_G711U == eType) || (PT_LPCM == eType) ||
                             (PT_G726 == eType)
                             //|| (PT_OPUS == eType)
                             || (PT_AAC == eType));
...
CAudioDecoder::CAudioDecoder(AX_U32 nChannel, const ADEC_CONFIG_T &tConfig) : m_nChannel(nChannel), m_stAdecConfig(tConfig) {
}
...
AX_BOOL CAudioDecoder::Restart(const ADEC_CONFIG_T &stAttr) {
    // TODO:
    if (stAttr.nBlkSize != m_stAdecConfig.nBlkSize || stAttr.nBlkCnt != m_stAdecConfig.nBlkCnt) {
        Stop();
...
AX_BOOL CAudioDecoder::Create() {
    ...
    // STEP 1: create adec channel
    {
        ...
        stChnAttr.u32MaxFrameSize = (0 == m_stAdecConfig.nBlkSize) ? ADEC_DEFAULT_BLK_SIZE : m_stAdecConfig.nBlkSize;

        if (PT_AAC == m_stAdecConfig.eType) {
            AX_U8 *pConf[] = {m_stAdecConfig.stDecoderAttr.stAacDecoder.ConfigBuf};
            stAacDecoderAttr = {.enTransType = m_stAdecConfig.stDecoderAttr.stAacDecoder.eTransType,
                                .u8Conf = pConf,
            ...
            stG726DecoderAttr = {.u32BitRate = m_stAdecConfig.stDecoderAttr.stDefDecoder.nBitRate};

            stChnAttr.pValue = &stG726DecoderAttr;
        } else if (PT_OPUS == m_stAdecConfig.eType) {
            stOpusDecoderAttr = {.u32SamplingRate = m_stAdecConfig.eSampleRate,
...
AX_VOID CAudioDecoder::StartWorkThread() {
    if (!m_stAdecConfig.bLink && !m_bGetThreadRunning) {
...
AX_VOID CAudioDecoder::FrameGetThreadFunc(CAudioDecoder *pCaller) {
    ...
    AX_CHAR szName[50] = {0};
    sprintf(szName, "APP_ADEC_Get_%d", nChannel);
...
AX_BOOL CAudioDecoder::SetAttr(const ADEC_CONFIG_T &stAttr) {
    if (m_bStart) {
        if (stAttr.bLink != m_stAdecConfig.bLink || stAttr.nChannel != m_stAdecConfig.nChannel ||
            stAttr.nBlkSize != m_stAdecConfig.nBlkSize || stAttr.nBlkCnt != m_stAdecConfig.nBlkCnt ||
            stAttr.nInDepth != m_stAdecConfig.nInDepth || stAttr.nOutDepth != m_stAdecConfig.nOutDepth ||
            stAttr.eType != m_stAdecConfig.eType || stAttr.eBitWidth != m_stAdecConfig.eBitWidth ||
            stAttr.eSampleRate != m_stAdecConfig.eSampleRate || stAttr.eSoundMode != m_stAdecConfig.eSoundMode) {
