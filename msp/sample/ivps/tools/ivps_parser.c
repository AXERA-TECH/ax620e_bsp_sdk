...
static int ivps_fmt_cfg_get(const char *str, AX_IMG_FORMAT_E *eDstPicFormat)
{
    ...
    if (!strcmp(str, "AX_FORMAT_YUV422_INTERLEAVED_YUYV"))
    {
        *eDstPicFormat = AX_FORMAT_YUV422_INTERLEAVED_YUYV;
        return 0;
    }
...
static int ivps_grp_cfg_get(const INI_DICT *ini, const char *sect, SAMPLE_IVPS_GRP_T *pGrpCfg)
{
    ...
    pGrpCfg->nIvpsGrp = ini_getint(ini, strcat_unfmt(key, sect, ":nIvpsGrp"), -1);
...
static int ivps_tdp_cfg_get(const INI_DICT *ini,
                            const char *sect, AX_IVPS_TDP_CFG_T *ptTdpCfg)
{
    char key[256];
    const char *str = NULL;

    ptTdpCfg->eRotation = ini_getint(ini, strcat_unfmt(key, sect, ":eRotation"), 0) / 90;

    if (ptTdpCfg->eRotation >= AX_IVPS_ROTATION_BUTT)
    {
        ALOGE("Invalid eRotation:%d", ptTdpCfg->eRotation);
        return -2;
    }
    printf("ptTdpCfg->eRotation:%d", ptTdpCfg->eRotation);

    str = ini_get_string(ini, strcat_unfmt(key, sect, ":bMirror"), "null");
    PARSER_LOGI("bMirror:%s", str);
    if (strcmp(str, "AX_TRUE"))
        ptTdpCfg->bMirror = AX_FALSE;
    else
        ptTdpCfg->bMirror = AX_TRUE;

    str = ini_get_string(ini, strcat_unfmt(key, sect, ":bFlip"), "null");
    PARSER_LOGI("bFlip:%s", str);
    if (strcmp(str, "AX_TRUE"))
        ptTdpCfg->bFlip = AX_FALSE;
    else
        ptTdpCfg->bFlip = AX_TRUE;

    str = ini_get_string(ini, strcat_unfmt(key, sect, ":bRotationOnly"), "null");
    PARSER_LOGI("bRotationOnly:%s", str);
    if (strcmp(str, "AX_TRUE"))
        ptTdpCfg->bRotationOnly = AX_FALSE;
    else
        ptTdpCfg->bRotationOnly = AX_TRUE;
    return 0;
}
                            ...
                            const char *sect, AX_IVPS_GDC_CFG_T *ptGdcCfg)
{
    ...
    ptGdcCfg->eRotation = ini_getint(ini, strcat_unfmt(key, sect, ":eRotation"), 0) / 90;
                               ...
                               const char *sect, AX_IVPS_FILTER_T *pFilterCfg)
{
    ...
    str = ini_get_string(ini, strcat_unfmt(key, sect, ":bEngage"), "AX_FALSE");
    ...
    if (!strcmp(str, "AX_IVPS_ENGINE_TDP"))
    {
        ...
        ivps_tdp_cfg_get(ini, "TDP", &pFilterCfg->tTdpCfg);
    }
    ...
    else if (!strcmp(str, "AX_IVPS_ENGINE_SCL"))
    {
        pFilterCfg->eEngine = AX_IVPS_ENGINE_SCL;
    }
    else
    {
        ALOGE("Invalid module: %s in ini, should be TDP,VPP,GDC,SCL", str);
    }

    str = ini_get_string(ini, strcat_unfmt(key, sect, ":bCrop"), "AX_FALSE");
    ...
    str = ini_get_string(ini, strcat_unfmt(key, sect, ":enCompressMode"), "AX_COMPRESS_MODE_NONE");
    ...
    str = ini_get_string(ini, strcat_unfmt(key, sect, ":eSclType"), "AX_IVPS_SCL_TYPE_AUTO");
    if (!strcmp(str, "AX_IVPS_SCL_TYPE_AUTO"))
    {
        pFilterCfg->eSclType = AX_IVPS_SCL_TYPE_AUTO;
    }
    else if (!strcmp(str, "AX_IVPS_SCL_TYPE_PHASE_FILTER"))
    {
        pFilterCfg->eSclType = AX_IVPS_SCL_TYPE_PHASE_FILTER;
    }
    else if (!strcmp(str, "AX_IVPS_SCL_TYPE_BILINEAR"))
    {
        pFilterCfg->eSclType = AX_IVPS_SCL_TYPE_BILINEAR;
    }
    else
    {
        ALOGE("Invalid eSclType:%s", str);
    }
    return 0;
}
...
int IVPS_DewarpIniParserType(const char *ininame)
{
    ...
    {
        return SAMPLE_IVPS_DEWARP_GDC;
    }
    if (!strcmp(module, "SYNC"))
    {
        return SAMPLE_IVPS_DEWARP_SYNC;
    }
    ALOGE("module:%s in ini is not support!", module);
    dictionary_del(ini);
    return SAMPLE_IVPS_DEWARP_NONE;
}
...
int IVPS_ChnInfoParser(const IVPS_ARG_T *ptArg, AX_IVPS_PIPELINE_ATTR_T *ptPpl)
{
    ...
    for (int i = 0; i < ptArg->nOutChnNum; i++)
    {
        ...
        if (i == 0)
        {
            ptPpl->tFilter[0][0].nDstPicWidth = ptArg->tChnInfo[i].nW;
            ptPpl->tFilter[0][0].nDstPicHeight = ptArg->tChnInfo[i].nH;
            ptPpl->tFilter[0][0].nDstPicStride = ALIGN_UP(ptArg->tChnInfo[i].nW, 32);
        }
        ptPpl->tFilter[i + 1][0].nDstPicWidth = ptArg->tChnInfo[i].nW;
        ptPpl->tFilter[i + 1][0].nDstPicHeight = ptArg->tChnInfo[i].nH;
        ptPpl->tFilter[i + 1][0].nDstPicStride = ALIGN_UP(ptArg->tChnInfo[i].nW, 32);
    }
    return 0;
}