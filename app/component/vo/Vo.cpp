...
AX_BOOL CVo::ShowNoVideo(AX_VOID) {
    ...
    for (AX_U32 i = ((m_rcLogo.bShow ? 1 : 0) + m_nVideoChnCount); i < m_arrChns.size(); ++i) {
        ...
        AX_IVPS_CROP_RESIZE_ATTR_T tResizeAttr;
        memset(&tResizeAttr, 0, sizeof(AX_IVPS_CROP_RESIZE_ATTR_T));
        tResizeAttr.tAspectRatio.eMode = AX_IVPS_ASPECT_RATIO_STRETCH;
        ret = AX_IVPS_CropResizeTdp(&src, &dst, &tResizeAttr);
        if (0 != ret) {
...
AX_BOOL CVo::DestoryPool(AX_POOL& pool) {
