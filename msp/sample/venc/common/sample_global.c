...
AX_S32 SampleReadFbcFile(FILE *pFileIn, AX_VIDEO_FRAME_T *pstFrame, SAMPLE_VENC_SENDFRAME_PARA_T *pstArg)
{
...
AX_VOID *SAMPLE_VENC_SendFrameProc(AX_VOID *arg)
{
    ...
    while (pstArg->bSendFrmStart) {
        ...
        if (pstArg->stFbcInfo.fbcType) {
            readSize = SampleReadFbcFile(fFileIn, &stFrame.stVFrame, pstArg);
        } else {
            /* read frame data from yuv file */
            readSize =
                COMMON_VENC_ReadFile(fFileIn, width, strideY, height, eFmt, (void *)(AX_ULONG)stFrame.stVFrame.u64VirAddr[0]);
        }
...
AX_VOID *SAMPLE_VENC_SendFramePerfProc(AX_VOID *arg)
{
    ...
    while (pstArg->bSendFrmStart) {
        ...
        if (0 == (totalInputFrames % pstArg->vbCnt)) {
            for (AX_S32 blk = pstArg->vbCnt - 1; blk >= 0; blk--) {
                if (AX_INVALID_BLOCKID != gBlkArr[blk])
                    AX_POOL_ReleaseBlock(gBlkArr[blk]);
            }
...
AX_VOID *SAMPLE_VENC_SendFrameQpMapProc(AX_VOID *arg)
{
    ...
    while (pstArg->bSendFrmStart) {
        ...
        stFrame.stUserFrame.stVFrame.u64VirAddr[0] = (AX_ULONG)AX_POOL_GetBlockVirAddr(blkId);
        ...
        QpMapVirAddr = (AX_S8 *)((AX_ULONG)stFrame.stUserFrame.stVFrame.u64VirAddr[0] + frameSize);
        ...
        if (pstArg->stFbcInfo.fbcType) {
            readSize = SampleReadFbcFile(fFileIn, &stFrame.stUserFrame.stVFrame, pstArg);
        } else {
            /* read frame data from yuv file */
            readSize = COMMON_VENC_ReadFile(fFileIn, width, strideY, height, eFmt,
                                            (void *)(AX_ULONG)stFrame.stUserFrame.stVFrame.u64VirAddr[0]);
        }
        ...
        stFrame.stUserFrame.stVFrame.u64PTS = USER_SET_PTS_VALUE;
        ...
        if (AX_SUCCESS != s32Ret) {
            ...
            if (AX_ERR_VENC_FLOW_END == s32Ret) {
                SAMPLE_LOG_WARN("chn-%d: AX_VENC_SendFrameEx AX_ERR_VENC_FLOW_END, ret=%x\n", VeChn, s32Ret);
                goto EXIT;
            }
...
AX_VOID *SAMPLE_VENC_SendFrameDynResolutionProc(AX_VOID *arg)
{
    ...
    while (pstArg->bSendFrmStart) {
        ...
        if (pstArg->stFbcInfo.fbcType) {
            readSize = SampleReadFbcFile(pstArg->fFileIn, &stFrame.stVFrame, pstArg);
        } else {
            /* read frame data from yuv file */
            ...
            if (bLoopEncode) {
                ...
                if (pstArg->stFbcInfo.fbcType) {
                    ...
                    readSize = COMMON_VENC_ReadFile(pstArg->fFileIn, pstArg->width, pstArg->strideY, pstArg->height,
                                                    eFmt, (void *)(AX_ULONG)stFrame.stVFrame.u64VirAddr[0]);
                }
            } else {
                SAMPLE_LOG_WARN("chn-%d: End of input file!\n", VeChn);
                /* no more frames, stop encoder */
...
AX_VOID *SAMPLE_VENC_GetStreamProc(AX_VOID *arg)
{
    ...
    while (pstArg->bGetStrmStart) {
        ...
        if (AX_SUCCESS == s32Ret) {
            ...
            if (AX_SUCCESS != s32Ret) {
                SAMPLE_LOG_ERR("chn-%d: AX_VENC_ReleaseStream failed, s32Ret = 0x%x!\n", VeChn, s32Ret);
                goto EXIT;
            }
