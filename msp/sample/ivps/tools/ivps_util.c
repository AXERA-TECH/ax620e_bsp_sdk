...
#define MESH_INSERT_BLANK_LINE
AX_BOOL LoadMeshHex(const char *pszImge, AX_U32 pBufSize, AX_U64 *nPhyAddr, AX_VOID **pVirAddr)
{
    AX_S32 ret;
    char *p, buf[20];
    AX_U64 value;
    ...
    if (ret)
    {
        ALOGE("Cmm alloc[%d] fail, ret=%d", nFileSize, ret);
        fclose(fp);
        return AX_FALSE;
    }
...
#ifdef MESH_INSERT_BLANK_LINE /* insert 3 lines 0 every 33 lines */
        if (line % 33 == 0 && line)
        {
            *pData = 0;
            pData++;
                 ...
                 char *pFilePath, char *pFileName)
{
    ...
    case 2:
        ...
        if (AX_FORMAT_YUV422_SEMIPLANAR == tDstFrame->enImgFormat || AX_FORMAT_YUV422_SEMIPLANAR_VU == tDstFrame->enImgFormat
            || AX_FORMAT_YUV422_SEMIPLANAR_10BIT_P101010 == tDstFrame->enImgFormat || AX_FORMAT_YUV422_SEMIPLANAR_10BIT_P010 == tDstFrame->enImgFormat)
        {
