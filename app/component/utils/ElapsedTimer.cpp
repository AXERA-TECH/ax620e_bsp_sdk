...
AX_VOID CElapsedTimer::Show(clock::time_point &end, AX_U32 nUnit, AX_CHAR const *pFmt, va_list vlist) {
    ...
    switch (nUnit) {
        case CElapsedTimer::seconds:
            printf("%s => elapsed: %d (s)\n", szBuf, (AX_U32)(chrono::duration_cast<chrono::seconds>(end - m_begin).count()));
            break;
        case CElapsedTimer::milliseconds:
            printf("%s => elapsed: %d (ms)\n", szBuf, (AX_U32)(chrono::duration_cast<chrono::milliseconds>(end - m_begin).count()));
            break;
        case CElapsedTimer::microseconds:
            printf("%s => elapsed: %lld (us)\n", szBuf, (AX_U64)(chrono::duration_cast<chrono::microseconds>(end - m_begin).count()));
            break;
        case CElapsedTimer::nanoseconds:
            printf("%s => elapsed: %lld (ns)\n", szBuf, (AX_U64)(chrono::duration_cast<chrono::nanoseconds>(end - m_begin).count()));
            break;
        default:
            printf("%s => elapsed: %d (ms)\n", szBuf, (AX_U32)(chrono::duration_cast<chrono::milliseconds>(end - m_begin).count()));
            break;
