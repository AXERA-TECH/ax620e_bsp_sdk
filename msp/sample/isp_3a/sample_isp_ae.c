...
// AX_S32 sample_ae_sns_set_param(AX_U8 pipe, AX_SNS_EXP_CTRL_PARAM_T *pSnsParam)
// {
//     AX_S32 result = 0;
...
)
{
    ...
    pCtxManual->fGain    = pCtxStatus->snsRegAgain[AXAE_LONG_FRAME];
    pCtxManual->fDgain   = pCtxStatus->snsDgain[AXAE_LONG_FRAME];
    ...
    pCtxManual->fIntegrationTime = max(pCtxStatus->snsShutter[AXAE_LONG_FRAME], pCtxConfig->IntegrationTimeLimit.fMin);
    AX_F32 lineNum = pCtxManual->fIntegrationTime / pCtxConfig->flineTime;
...
)
{
    ...
    pAlgCtx->tStatus.snsDgain[AXAE_LONG_FRAME]        = snsDgain;
    pAlgCtx->tStatus.snsRegAgain[AXAE_LONG_FRAME]     = snsRegAgain;
    pAlgCtx->tStatus.snsTotalRegGain[AXAE_LONG_FRAME] = snsTotalRegGain;
    pAlgCtx->tStatus.snsTotalAgain[AXAE_LONG_FRAME]   = snsTotalAgain;
    pAlgCtx->tStatus.snsTotalGain[AXAE_LONG_FRAME]    = snsTotalGain;
    pAlgCtx->tStatus.totalGain[AXAE_LONG_FRAME]       = totalGain;
    pAlgCtx->tStatus.snsShutter[AXAE_LONG_FRAME]      = snsShutter;
    pAlgCtx->tStatus.totalExposure[AXAE_LONG_FRAME]   = totalExposure;
    ...
    pAlgCtx->tStatus.hdrHwMaxShutter = tSnsParam.sns_ae_limit.tIntTimeRange.fMaxIntegrationTime[AXAE_LONG_FRAME] * tSnsParam.fTimePerLine;
    pAlgCtx->tStatus.algoMaxShutter = min(pAlgCtx->tStatus.hdrHwMaxShutter, (AX_F32)pAlgCtx->tConfig.maxUserShutter);
    ...
    if (hdrMode == AX_SNS_LINEAR_MODE) {
        ...
        // Update Context's Exposure Settings
        pAlgCtx->tStatus.snsDgain[AXAE_MEDIUM_FRAME] 	   = shortDgain;
        pAlgCtx->tStatus.snsRegAgain[AXAE_MEDIUM_FRAME]	   = shortRegAgain;
        pAlgCtx->tStatus.snsTotalRegGain[AXAE_MEDIUM_FRAME] = shortTotalRegGain;
        pAlgCtx->tStatus.snsTotalAgain[AXAE_MEDIUM_FRAME]   = shortTotalAgain;
        pAlgCtx->tStatus.snsTotalGain[AXAE_MEDIUM_FRAME]    = shortSnsTotalGain;
        pAlgCtx->tStatus.totalGain[AXAE_MEDIUM_FRAME]	   = shortTotalGain;
        pAlgCtx->tStatus.snsShutter[AXAE_MEDIUM_FRAME]	   = shortShutter;
        pAlgCtx->tStatus.totalExposure[AXAE_MEDIUM_FRAME]   = shortTotalExposure;
        ...
        // Update Context's Exposure Settings
        pAlgCtx->tStatus.snsDgain[AXAE_SHORT_FRAME] 	   = vsDgain;
        pAlgCtx->tStatus.snsRegAgain[AXAE_SHORT_FRAME]	   = vsRegAgain;
        pAlgCtx->tStatus.snsTotalRegGain[AXAE_SHORT_FRAME] = vsTotalRegGain;
        pAlgCtx->tStatus.snsTotalAgain[AXAE_SHORT_FRAME]   = vsTotalAgain;
        pAlgCtx->tStatus.snsTotalGain[AXAE_SHORT_FRAME]    = vsSnsTotalGain;
        pAlgCtx->tStatus.totalGain[AXAE_SHORT_FRAME]	   = vsTotalGain;
        pAlgCtx->tStatus.snsShutter[AXAE_SHORT_FRAME]	   = vsShutter;
        pAlgCtx->tStatus.totalExposure[AXAE_SHORT_FRAME]   = vsTotalExposure;
...
(
    ...
    pCtxConfig->IntegrationTimeLimit.fStep = tSnsParam.sns_ae_param.fIntegrationTimeIncrement[AXAE_LONG_FRAME] * tSnsParam.fTimePerLine;
    ...
    if( tSnsParam.fTimePerLine == 0 )
    {
        printf("%s, AE Get Line Period Failed !\n", __func__);
    }else{
...
{
    pAeResult->nIntTime[AXAE_LONG_FRAME]  = pStatus->snsShutter[AXAE_LONG_FRAME];
    pAeResult->nAgain[AXAE_LONG_FRAME]    = sample_ae_float_convert_to_int(pStatus->snsTotalAgain[AXAE_LONG_FRAME], 22, 10, false);
    pAeResult->nDgain[AXAE_LONG_FRAME]    = sample_ae_float_convert_to_int(pStatus->snsDgain[AXAE_LONG_FRAME], 22, 10, false);
    pAeResult->nTotalGain[AXAE_LONG_FRAME]    = sample_ae_float_convert_to_int(pStatus->totalGain[AXAE_LONG_FRAME], 22, 10, false);

    pAeResult->nIntTime[AXAE_MEDIUM_FRAME]  = pStatus->snsShutter[AXAE_MEDIUM_FRAME];
    pAeResult->nAgain[AXAE_MEDIUM_FRAME]    = sample_ae_float_convert_to_int(pStatus->snsTotalAgain[AXAE_MEDIUM_FRAME], 22, 10, false);
    pAeResult->nDgain[AXAE_MEDIUM_FRAME]    = sample_ae_float_convert_to_int(pStatus->snsDgain[AXAE_MEDIUM_FRAME], 22, 10, false);
    pAeResult->nTotalGain[AXAE_MEDIUM_FRAME]    = sample_ae_float_convert_to_int(pStatus->totalGain[AXAE_MEDIUM_FRAME], 22, 10, false);

    pAeResult->nIntTime[AXAE_SHORT_FRAME]  = pStatus->snsShutter[AXAE_SHORT_FRAME];
    pAeResult->nAgain[AXAE_SHORT_FRAME]    = sample_ae_float_convert_to_int(pStatus->snsTotalAgain[AXAE_SHORT_FRAME], 22, 10, false);
    pAeResult->nDgain[AXAE_SHORT_FRAME]    = sample_ae_float_convert_to_int(pStatus->snsDgain[AXAE_SHORT_FRAME], 22, 10, false);
    pAeResult->nTotalGain[AXAE_SHORT_FRAME]    = sample_ae_float_convert_to_int(pStatus->totalGain[AXAE_SHORT_FRAME], 22, 10, false);
    ...
    if (nHdrMode == 1){
        printf("AE[pipe=%d] Result: shutter=%f, Again=%.2f, ispGain=%.2f, lux=%.2f, Luma=%.2f \n",
            pipe, pStatus->snsShutter[AXAE_LONG_FRAME],  pStatus->snsTotalAgain[AXAE_LONG_FRAME],
...
)
{
    ...
    // tSnsParam.sns_ae_limit.fMingain = 1.000000;
    // tSnsParam.sns_ae_limit.fMaxgain = 991.380005;
    // tSnsParam.sns_ae_limit.fMingain = 1.000000;
    // tSnsParam.sns_ae_limit.fMingain = 1.000000;
    // tSnsParam.sns_ae_limit.fMingain = 1.000000;
    ...
    for(int i = 0 ;i<tSnsParam.sns_dev_attr.eSnsMode;i++){
        ...
        tSnsParam.sns_ae_limit.tIntTimeRange.fMaxIntegrationTime[i] = 33000 / tSnsParam.fTimePerLine;
        tSnsParam.sns_ae_param.fCurAGain[i] = 2.0f;
        tSnsParam.sns_ae_param.fCurDGain[i] = 1.0f;
        tSnsParam.sns_ae_param.fCurIspDGain[i] = 1.0f;
        tSnsParam.sns_ae_param.fCurIntegrationTime[i] = 10000 / tSnsParam.fTimePerLine;
        ...
        tSnsParam.sns_ae_param.fIntegrationTimeIncrement[i] = 1;
    ...
    // set again, dgain, uHcg_lcg, shutter
    pCtxStatus->snsDgain[AXAE_LONG_FRAME] = 1;
    pCtxStatus->snsRegAgain[AXAE_LONG_FRAME] = 5;
    pCtxStatus->snsShutter[AXAE_LONG_FRAME] = 10000;    // 10ms
    pCtxStatus->totalGain[AXAE_LONG_FRAME] = 5.0;
...
)
{
    ...
    for (AX_S32 i = 0; i < nZoneRow; i++) {
        for (AX_S32 j = 0; j < nZoneCol; j++) {
            AX_F32 tempR_sum = (float)(luma_data[i * nZoneCol + j].nGridSum[0] >> 4) +
                               (luma_data[i * nZoneCol + j].nGridSum[0] & 0x0F) / 16.0;
            AX_F32 tempB_sum = (float)(luma_data[i * nZoneCol + j].nGridSum[1] >> 4) +
                               (luma_data[i * nZoneCol + j].nGridSum[1] & 0x0F) / 16.0;
            AX_F32 tempGr_sum = (float)(luma_data[i * nZoneCol + j].nGridSum[2] >> 4) +
                                (luma_data[i * nZoneCol + j].nGridSum[2] & 0x0F) / 16.0;
            AX_F32 tempGb_sum = (float)(luma_data[i * nZoneCol + j].nGridSum[3] >> 4) +
                                (luma_data[i * nZoneCol + j].nGridSum[3] & 0x0F) / 16.0;
...
)
{
    ...
    pCtxStatus->AxPreEV[snsDelayFrame] = pCtxStatus->totalExposure[AXAE_LONG_FRAME];
...
)
{
     ...
     * in dark environment, it will not reach the maximum integration time due to UI accuracy(U32).
     */
    AX_F32 lineNum = SplitIntegrationTime / pCtxConfig->flineTime;
...
)
{
    ...
    pAlgCtx->tStatus.lux =   pAlgCtx->tStatus.meanLuma * pAlgCtx->tConfig.luxK
                             / pAlgCtx->tStatus.totalExposure[AXAE_LONG_FRAME];
