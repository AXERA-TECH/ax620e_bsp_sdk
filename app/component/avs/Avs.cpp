...
AX_BOOL CAvs::Init(const AX_APP_AVS_ATTR_T& stAvsAttr) {
    ...
    return LoadParam(m_stAvsAttr.tCaliInitParam.strCaliDataPath);
}
...
AX_BOOL CAvs::LoadParam(string strCaliDataPath) {
    AX_S32 s32Ret { -1 };
    if (strCaliDataPath.empty()) {
        LOG_M_E(TAG, "strCaliDataPath is empty.");
        return AX_FALSE;
    }

    s32Ret = AX_AVSCALI_LoadParam(strCaliDataPath.c_str(), &m_stAvsGrpAttr, &m_t3ASyncRatio, &m_bIsCalibrated);
    if (AX_SUCCESS != s32Ret) {
    ...
    LOG_MM_N(TAG, "AVS out resolution:%dX%d", m_stAvsResolution.u32Width, m_stAvsResolution.u32Height);
