...
void* RtspServerThreadFunc(void *args)
{
    ...
    for (AX_S32 i = 0; i < nMaxNum; i++) {
        AX_CHAR strStream[64] = {0};
        sprintf(strStream, "axstream%d", stRtspAttr[i].nChannel);
        pRtspServer->deleteServerMediaSession(strStream);
        for (AX_U32 j = 0; j < RTSP_MEDIA_BUTT; j ++) {
            pLiveServerMediaSession[i][j] = nullptr;
        }
    }

    RTSPServer::close(pRtspServer);
    pRtspServer = nullptr;
    pUEnv->reclaim();
    pUEnv = nullptr;
