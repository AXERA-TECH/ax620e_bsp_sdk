...
AX_VOID CAppLogWrapper::LogArgStr(AX_S32 nLv, const AX_CHAR* pFmt, va_list vlist) {
    ...
    if (nLen > 0) {
        ...
        localtime_r(&now, &t);
#if 1
        snprintf(szLog, sizeof(szLog), "%02u-%02u %02u:%02u:%02u:%03u %llu %6lu %s", t.tm_mon + 1, t.tm_mday, t.tm_hour, t.tm_min, t.tm_sec,
                 (AX_U32)(tv.tv_usec / 1000), GetTickCount(), (long unsigned int)gettid(), szBuf);
#else
