...
{
    AX_U64 set_val1 = 0,set_val2 = 0,set_val3 = 0,set_val4 = 0;
    cJSON *root = NULL;
    ...
    case 2:
    case 3:
        item = cJSON_GetObjectItem(root, "set_val1");
        SAMPLE_IVE_CHECK_EXPR_GOTO(!item, PARSE_FAIL, "Error:parse param set_val failed!\n");
        set_val1 = item->valueint;
        item = cJSON_GetObjectItem(root, "set_val2");
        SAMPLE_IVE_CHECK_EXPR_GOTO(!item, PARSE_FAIL, "Error:parse param set_val failed!\n");
        set_val2 = item->valueint;
        item = cJSON_GetObjectItem(root, "set_val3");
        SAMPLE_IVE_CHECK_EXPR_GOTO(!item, PARSE_FAIL, "Error:parse param set_val failed!\n");
        set_val3 = item->valueint;
        item = cJSON_GetObjectItem(root, "set_val4");
        SAMPLE_IVE_CHECK_EXPR_GOTO(!item, PARSE_FAIL, "Error:parse param set_val failed!\n");
        set_val4 = item->valueint;
        pstTestDma->stDmaCtrl.u64Val = ((set_val4 & 0xFFFF) << 48) | ((set_val3 & 0xFFFF) << 32) | ((set_val2 & 0xFFFF) << 16) | (set_val1 & 0xFFFF);
    ...
    // while (1) {
        ...
        // if (cnt % 3 == 0) {
            ...
            while (AX_ERR_IVE_QUERY_TIMEOUT == s32Ret) {
                usleep(1000*100);
                SAMPLE_IVE_PRT("AX_IVE_Query(IveHandle:%d) timeout, retry...\n", IveHandle);
                s32Ret = AX_IVE_Query(IveHandle, &bFinish, bBlock);
            }
            // if (AX_SUCCESS != s32Ret) {
