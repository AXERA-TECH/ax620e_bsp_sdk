...
AX_BOOL CAudioEncoder::Create() {
    ...
    if (PT_AAC == m_stAencConfig.eType) {
        ...
        stOpusEncoderAttr = {.enApplication = AX_OPUS_APPLICATION_RESTRICTED_LOWDELAY,
...
AX_VOID CAudioEncoder::FrameGetThreadFunc(CAudioEncoder* pCaller) {
    ...
    AX_CHAR szName[50] = {0};
    sprintf(szName, "APP_AENC_Get_%d", nChannel);
...
AX_BOOL CAudioEncoder::SetAttr(const AENC_CONFIG_T& stAttr) {
    // TODO
    if (m_bStart) {
        if (stAttr.bLink != m_stAencConfig.bLink || stAttr.nChannel != m_stAencConfig.nChannel ||
            stAttr.nInDepth != m_stAencConfig.nInDepth || stAttr.nOutDepth != m_stAencConfig.nOutDepth ||
            stAttr.nChnCnt != m_stAencConfig.nChnCnt || stAttr.nBitRate != m_stAencConfig.nBitRate ||
            stAttr.eType != m_stAencConfig.eType || stAttr.eBitWidth != m_stAencConfig.eBitWidth ||
            stAttr.eSampleRate != m_stAencConfig.eSampleRate || stAttr.eSoundMode != m_stAencConfig.eSoundMode) {
