...
CAudioFilePlay::CAudioFilePlay(CAudioPlayDev *pAudioPlayDev, CAudioDecoder *pAudioDecoder, AX_U32 nChannel)
    : m_nChannel(nChannel), m_pAudioPlayDev(pAudioPlayDev), m_pAudioDecoder(pAudioDecoder) {
}
...
AX_BOOL CAudioFilePlay::Play(AX_PAYLOAD_TYPE_E eType, const std::string &strFileName, AX_S32 nLoop, AUDIO_FILE_PLAY_CALLBACK callback,
                             AX_VOID *pUserData) {
...
AX_VOID CAudioFilePlay::ResultCallbackThread(AX_VOID) {
    AX_CHAR szName[50] = {0};
    sprintf(szName, "APP_AFPLAY_Res_%d", m_nChannel);
...
AX_VOID CAudioFilePlay::ProcessFileThread() {
    AX_CHAR szName[50] = {0};
    sprintf(szName, "APP_AFPLAY_%d", m_nChannel);
    prctl(PR_SET_NAME, szName);
...
AX_VOID CAudioFilePlay::ProcessPlayFile(AUDIO_FILE_PLAY_ATTR_PTR pstPlayFileAttr) {
    ...
    pBuffer = (AX_U8 *)AX_MALLOC(nReadSize);
        ...
        do {
            ...
            if (PT_AAC == pstPlayFileAttr->eType) {
                ...
                if (AX_AAC_TRANS_TYPE_ADTS == eTransType) {
                    ...
                    if (nNumRead < 1 || nPacketSize > nReadSize) {
                        pstPlayFileAttr->eStatus = AUDIO_FILE_PLAY_STATUS_ERROR;
                        LOG_MM_E(AFPLAY, "Invalid packet size(%d)", nPacketSize);
                        break;
                    }
            ...
            if (nBufferSize > 0) {
                ...
                // TODO:
                CElapsedTimer::GetInstance()->mSleep(30);
            } else {
                break;
            }
    ...
    } while (m_bPlayFileThreadRunning && !m_bStopPlayFile && (pstPlayFileAttr->nLoop < 0 || (nLoop < pstPlayFileAttr->nLoop)) &&
             pstPlayFileAttr->eStatus != AUDIO_FILE_PLAY_STATUS_ERROR);

    if (m_bStopPlayFile) {
        pstPlayFileAttr->eStatus = AUDIO_FILE_PLAY_STATUS_STOP;
    } else if (nLoop >= pstPlayFileAttr->nLoop && pstPlayFileAttr->eStatus != AUDIO_FILE_PLAY_STATUS_ERROR) {
