...
#define ADEC_META_SIZE (4 * 1024)
#define ADEC_AAC_BLK_SIZE (4 * 1024)
#define ADEC_OPUS_BLK_SIZE (38400)
#define ADEC_G711_BLK_SIZE (4 * 1024)
#define ADEC_DEFAULT_BLK_SIZE (4 * 1024)
#define ADEC_DEFAULT_BLK_CNT (34)
...
typedef struct adecConfig {
    ...
    adecConfig(AX_PAYLOAD_TYPE_E eAdecType = PT_G711A) {
        ...
        if (PT_AAC == eType) {
            ...
            stDecoderAttr.stAacDecoder.eTransType = AX_AAC_TRANS_TYPE_ADTS;
            stDecoderAttr.stAacDecoder.nConfLen = 0;

        } else {
            nBlkSize = ADEC_DEFAULT_BLK_SIZE;
            nBlkCnt = ADEC_DEFAULT_BLK_CNT;
            eSoundMode = AX_AUDIO_SOUND_MODE_MONO;
            stDecoderAttr.stDefDecoder.nBitRate = 32000;
            stDecoderAttr.stDefDecoder.eBitWidth = eBitWidth;
            stDecoderAttr.stDefDecoder.eSampleRate = eSampleRate;
        }
    }
