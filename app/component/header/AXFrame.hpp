...
typedef enum axAPP_FRAME_TYPE { AX_APP_FRAME_TYPE_VIDEO, AX_APP_FRAME_TYPE_AUDIO, AX_APP_FRAME_TYPE_BUTT } AX_APP_FRAME_TYPE_E;
...
public:
    ...
    AX_BOOL LoadFile(const char *pFile) {
        ...
        if (0 == stFrame.stVFrame.stVFrame.u32Width || 0 == stFrame.stVFrame.stVFrame.u32Height ||
            0 == stFrame.stVFrame.stVFrame.u32FrameSize || 0 == stFrame.stVFrame.stVFrame.u64VirAddr[0]) {
            return AX_FALSE;
        }
    ...
    AX_BOOL SaveFile(const char *pPath) const {
        ...
        if (0 == ::stat(pPath, &st) && S_ISDIR(st.st_mode)) {
            AX_CHAR szName[64];
            sprintf(szName, "dump_%dx%d_grp%d_chn%d_frame%lld.img", stFrame.stVFrame.stVFrame.u32PicStride[0],
                    stFrame.stVFrame.stVFrame.u32Height, nGrp, nChn, stFrame.stVFrame.stVFrame.u64SeqNum);
