...
typedef struct aencConfig {
    ...
    aencConfig(AX_PAYLOAD_TYPE_E eAencType = PT_G711A) {
        ...
        if (PT_AAC == eType) {
            ...
            // stEncoderAttr = {.stAacEncoder = {.eAacType = AX_AAC_TYPE_AAC_LC, .eTransType = AX_AAC_TRANS_TYPE_ADTS}};
            stEncoderAttr.stAacEncoder.eAacType = AX_AAC_TYPE_AAC_LC;
            stEncoderAttr.stAacEncoder.eTransType = AX_AAC_TRANS_TYPE_ADTS;

        } else {
            eSoundMode = AX_AUDIO_SOUND_MODE_MONO;
            // stEncoderAttr = {.stDefEncoder = {.nBitRate = nBitRate, .eBitWidth = eBitWidth, .eSampleRate = eSampleRate}};
            stEncoderAttr.stDefEncoder.nBitRate = nBitRate;
            stEncoderAttr.stDefEncoder.eBitWidth = eBitWidth;
            stEncoderAttr.stDefEncoder.eSampleRate = eSampleRate;
        }
    }
...
public:
    ...
    AX_BOOL GetAttr(AENC_CONFIG_T& stAttr);
    AX_BOOL SetAttr(const AENC_CONFIG_T& stAttr);
    AX_BOOL GetAacEncoderConfigBuf(const AX_U8** ppConfBuf, AX_U32* pDataSize);
