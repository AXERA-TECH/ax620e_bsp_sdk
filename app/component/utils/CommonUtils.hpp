...
public:
    ...
    static std::string GetPPLConfigDir(AX_BOOL bDefault = AX_FALSE) {
        AX_S32 nIndex = 0;
        ...
        switch (nIndex) {
            case E_PPL_TYPE_IPC:
                return "/opt/bin/res/ipc/config";
            case E_PPL_TYPE_PANO:
                return "/opt/bin/res/pano/config";
            default:
                return "";
        }
#else
        std::string configPath = GetConfigPath(bDefault);
        switch (nIndex) {
            case E_PPL_TYPE_IPC:
                return (configPath + "/ipc");
            case E_PPL_TYPE_PANO:
                return (configPath + "/pano");
            default:
                return "";
        }
#endif
    ...
    static std::string GetConfigPath(AX_BOOL bDefault) {
        std::string configPath = "./config";
        if (bDefault || !CCmdLineParser::GetInstance()->GetConfigPath(configPath)) {
            return configPath;
        }
    ...
    static AX_BOOL LoadImage(const AX_CHAR* pszImge, AX_VOID** ppVirAddr, AX_U32 nImgSize) {
        ...
        if (fp) {
            ...
            fseek(fp, 0, SEEK_SET);
            *ppVirAddr = AX_MALLOC(nFileSize);
            if (*ppVirAddr == nullptr) {
                LOG_M_E(UTILS, "malloc failed");
                fclose(fp);
                return AX_FALSE;
            }
            if (fread(*ppVirAddr, 1, nFileSize, fp) != nFileSize) {
    ...
    static AX_BOOL GetIP(AX_CHAR* pOutIPAddress) {
        ...
        for (size_t i = 0; i < vNetType.size(); i++) {
            for (char c = '0'; c <= '9'; ++c) {
                std::string strDevice = vNetType[i] + c;
                int fd;
                int ret;
                struct ifreq ifr;
