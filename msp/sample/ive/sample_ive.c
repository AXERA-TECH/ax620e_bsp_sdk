...
static AX_S32 SAMPLE_IVE_TestMultiCalcProc(TEST_MULTI_CALC_T* pstTestMultiCalc)
{
    ...
    pu32Hist = (AX_U32*)((AX_ADDR)pstTestMultiCalc->stHist.u64VirAddr);
    ...
    if (SIGINT == s32Signo || SIGTERM == s32Signo) {
        ...
        if(sem_trywait(&g_sem_lock)==0) {
            ...
            case 9:
                ...
                break;
            case 10:
                SAMPLE_IVE_GMM2_TEST_HandleSig();
                break;
            case 11:
            ...
            case 13:
                ...
                break;
            case 14://map
                SAMPLE_IVE_Map_TEST_HandleSig();
                break;
            case 15://lbp
                SAMPLE_IVE_Lbp_TEST_HandleSig();
                break;
            case 16://OrdStatFltBern
                SAMPLE_IVE_OrdStatFltBern_TEST_HandleSig();
                break;
            case 17://Corner
                SAMPLE_IVE_Corner_TEST_HandleSig();
                break;
            case 18://LkOpticalFlow
                SAMPLE_IVE_LkOpticalFlow_TEST_HandleSig();
                break;
            case 19://Ncc
                SAMPLE_IVE_Ncc_TEST_HandleSig();
                break;
            case 20://Sad
                SAMPLE_IVE_Sad_TEST_HandleSig();
                break;
            case 21://LkOpticalFlowPyr
                SAMPLE_IVE_LkOpticalFlowPyr_TEST_HandleSig();
                break;
            default :
                break;
            }
...
AX_VOID SAMPLE_IVE_Usage(AX_CHAR* pchPrgName)
{
    ...
    printf("\t\t14-Map.\n");
    printf("\t\t15-Lbp.\n");
    printf("\t\t16-OrdStatFltBern.\n");
    printf("\t\t17-Corner.\n");
    printf("\t\t18-LkOpticalFlow.\n");
    printf("\t\t19-Ncc.\n");
    printf("\t\t20-Sad.\n");
    printf("\t\t21-LkOpticalFlowPyr.\n");
    ...
    printf("\t-o | --output_files:Output image files or dir, if there are multiple outputs, separated by spaces\n"
        "\t\tNote:for DMA, blob of CCL case must be specified as directory.\n");
    ...
    printf("\t\tNote:\n"
        ...
        "\t\t  2. For MagAng, Multi Calc, no need control parameters.\n");
...
int main(int argc, char *argv[])
{
    ...
    AX_CHAR *pchPathSrc[16] = {NULL, NULL, NULL, NULL};
    ...
    while ((s32Ret = getopt_long(argc, argv, optstr, long_options, NULL)) != -1) {
        ...
        case 't':
            ...
            if (u32Index != 6) {
                ...
                if (u32Index == 1 || (u32Index == 2 && u32Mode == 0)
                    || u32Index == 8 || u32Index == 10 || u32Index == 13 || u32Index == 17) {
                    if ((optind + 1) <= (argc -1))
        ...
        case 'i':
            ...
            if (u32Index == 1 || (u32Index == 2 && u32Mode == 0)
                || u32Index == 8 || u32Index == 10 || u32Index == 16 || u32Index == 14
                || u32Index == 17 || u32Index == 19 || u32Index == 20) {
                if (optind <= (argc - 1))
            ...
            if (u32Index == 18) {
                if (optind <= (argc - 1))
                    pchPathSrc[1] = argv[optind];
                if (optind + 1 <= (argc - 1))
                    pchPathSrc[2] = argv[optind + 1];
                if (optind + 2 <= (argc - 1))
                    pchPathSrc[3] = argv[optind + 2];
            }
            if (u32Index == 21) {
                if (optind <= (argc - 1))
                    pchPathSrc[1] = argv[optind];
                ...
                if (optind + 2 <= (argc - 1))
                    pchPathSrc[3] = argv[optind + 2];
                if (optind + 3 <= (argc - 1))
                    pchPathSrc[4] = argv[optind + 3];
                if (optind + 4 <= (argc - 1))
                    pchPathSrc[5] = argv[optind + 4];
                if (optind + 5 <= (argc - 1))
                    pchPathSrc[6] = argv[optind + 5];
                if (optind + 6 <= (argc - 1))
                    pchPathSrc[7] = argv[optind + 6];
            }
            break;
        case 'o':
            pchPathDst[0] = argv[optind - 1];
            if (u32Index == 3 || u32Index == 8 || u32Index == 10 || u32Index == 17 || u32Index == 18 || u32Index == 20) {
                if (optind <= (argc - 1))
    ...
    case 9://Sobel
        ...
        break;
    case 10://GMM2
        SAMPLE_IVE_GMM2_TEST(as32Type, pchPathSrc, pchPathDst, u32WidthSrc, u32HeightSrc, pchParamsList);
        break;
    case 11://Thresh
    ...
    case 13://MultiCalc
        ...
        break;
    case 14://Map
        SAMPLE_IVE_Map_TEST(as32Type, pchPathSrc, pchPathDst[0], u32WidthSrc, u32HeightSrc, pchParamsList);
        break;
    case 15://Lbp
        SAMPLE_IVE_Lbp_TEST(as32Type, pchPathSrc[0], pchPathDst[0], u32WidthSrc, u32HeightSrc, pchParamsList);
        break;
    case 16://OrdStatFltBern
        SAMPLE_IVE_OrdStatFltBern_TEST(as32Type, pchPathSrc[0], pchPathDst[0], u32WidthSrc, u32HeightSrc, pchParamsList);
        break;
    case 17://Corner
        SAMPLE_IVE_Corner_TEST(as32Type, pchPathSrc, pchPathDst, u32WidthSrc, u32HeightSrc, pchParamsList);
        break;
    case 18://LkOpticalFlow
        SAMPLE_IVE_LkOpticalFlow_TEST(as32Type, pchPathSrc, pchPathDst, u32WidthSrc, u32HeightSrc, pchParamsList);
        break;
    case 19://Ncc
        SAMPLE_IVE_Ncc_TEST(as32Type, pchPathSrc, pchPathDst, u32WidthSrc, u32HeightSrc);
        break;
    case 20://Sad
        SAMPLE_IVE_Sad_TEST(as32Type, pchPathSrc, pchPathDst, u32WidthSrc, u32HeightSrc, pchParamsList);
        break;
    case 21://LkOpticalFlowPyr
        SAMPLE_IVE_LkOpticalFlowPyr_TEST(as32Type, pchPathSrc, pchPathDst, u32WidthSrc, u32HeightSrc, pchParamsList);
        break;
    default :
