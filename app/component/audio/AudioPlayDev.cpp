...
static AX_BOOL IsDnVqeEnabled(const AX_AP_DNVQE_ATTR_T &stVqeAttr) {
    return (AX_BOOL)((stVqeAttr.stNsCfg.bNsEnable != AX_FALSE) || (stVqeAttr.stAgcCfg.bAgcEnable != AX_FALSE));
}
}  // namespace

CAudioPlayDev::CAudioPlayDev(const AUDIO_PLAY_DEV_ATTR_T &stAttr) : m_stPlayDevAttr(stAttr) {
...
AX_BOOL CAudioPlayDev::Create() {
    ...
    stAttr.u32PeriodCount = AUDIO_PLAY_DEV_PERIOD_COUNT;
...
AX_BOOL CAudioPlayDev::SetAttr(const AUDIO_PLAY_DEV_ATTR_T &stAttr) {
    if (m_bStart) {
        if (stAttr.bLink != m_stPlayDevAttr.bLink || stAttr.nCardId != m_stPlayDevAttr.nCardId ||
            stAttr.nDeviceId != m_stPlayDevAttr.nDeviceId || stAttr.nDepth != m_stPlayDevAttr.nDepth ||
            stAttr.nChnCnt != m_stPlayDevAttr.nChnCnt || stAttr.nPeriodSize != m_stPlayDevAttr.nPeriodSize ||
            stAttr.eBitWidth != m_stPlayDevAttr.eBitWidth || stAttr.eSampleRate != m_stPlayDevAttr.eSampleRate ||
...
AX_BOOL CAudioPlayDev::SetResample(const AX_AUDIO_SAMPLE_RATE_E &eSampleRate) {
    ...
    if (eSampleRate != m_stPlayDevAttr.eSampleRate && !m_stResampleAttr.bEnable) {
    ...
    else if (eSampleRate == m_stPlayDevAttr.eSampleRate && m_stResampleAttr.bEnable) {
        m_stResampleAttr.bEnable = AX_FALSE;
        m_stResampleAttr.eSampleRate = eSampleRate;
