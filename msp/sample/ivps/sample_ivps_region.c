...
/*
 * Region: General term for Mask and OSD.
 * Mask: Privacy Mask.
 * OSD: On-Screen Display.
 * Region contains Mask and OSD.
 * Mask contains Cover(Line, Rect and Polygon) and Mosaic.
 */
...
static AX_VOID *IVPS_UpdateCanvasThread(AX_VOID *pArg)
{
    AX_S32 nRegionNum = (AX_LONG)pArg;
    if (!nRegionNum)
    {
        return (AX_VOID *)1;
    }
...
{
    if (nRegionNum > sizeof(gRegions) / sizeof(gRegions[0]))
    {
        ALOGE("nRegionNum[%d] is exceeding the set range!", nRegionNum);
        return -1;
    }
    for (AX_S32 i = 0; i < nRegionNum; i++)
    {
        ...
        if (IVPS_SUCC != ret)
        {
            ...
            for (; i >= 0; i--)
            {
                if (AX_IVPS_INVALID_REGION_HANDLE != gRegions[i].handle)
                {
                    AX_IVPS_RGN_DetachFromFilter(gRegions[i].handle,
                                                 gRegions[i].IvpsGrp, gRegions[i].IvpsFilter);
