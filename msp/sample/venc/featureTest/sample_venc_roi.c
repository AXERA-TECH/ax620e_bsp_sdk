...
static AX_S32 SampleVencReadRoimap(AX_CHAR *roimapFile, AX_VENC_ROI_ATTR_T *pstRoiAttr)
{
    ...
    while ((fgets(buf, 50, fpROI) != NULL) && roiRectNum < AX_MAX_VENC_ROI_NUM) {
        if (buf[0] == 'r') {
...
AX_S32 SampleVencRoi(AX_S32 VencChn, AX_VOID *handle)
{
    ...
    int i = 0;
    AX_VENC_ROI_ATTR_T stRoiAttr[AX_MAX_VENC_ROI_NUM];
    ...
    memset(&stRoiAttr[0], 0x0, sizeof(AX_VENC_ROI_ATTR_T) * AX_MAX_VENC_ROI_NUM);
    ...
    for (i = 0; i < AX_MAX_VENC_ROI_NUM; i++) {
