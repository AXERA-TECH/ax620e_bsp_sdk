    ...
    AX_CHAR* pchDstFileName,  AX_CHAR* pchDstMemInfoName, AX_U32 u32Width, AX_U32 u32Height, AX_S32 as32Type[])
{
    ...
    snprintf(FileNamePrefix, 127, "%s/ccl_blob_mode%d_%dx%d", pchDstMemInfoName, pstTestCcl->stCclCtrl.enMode, pstTestCcl->stSrc.u32Width, pstTestCcl->stSrc.u32Height);
    for (AX_S32 i = 0; i < 6; i++) {
        if (i == 0)
...
static AX_VOID SAMPLE_IVE_CCL_OutImageConvert(const AX_IVE_IMAGE_T* pstSrcImg, AX_IVE_IMAGE_T* pstDstImg)
{
    ...
    for (AX_S32 i = 0, j = 0; i < size; i+=3, j++) {
        *((AX_U8 *)((AX_ADDR)pstDstImg->au64VirAddr[0]) + i) = (*((AX_U32 *)((AX_ADDR)pstSrcImg->au64VirAddr[0]) + j)) & 0xFF;
        *((AX_U8 *)((AX_ADDR)pstDstImg->au64VirAddr[0]) + i + 1) = (*((AX_U32 *)((AX_ADDR)pstSrcImg->au64VirAddr[0]) + j) >> 8) & 0xFF;
        *((AX_U8 *)((AX_ADDR)pstDstImg->au64VirAddr[0]) + i + 2) = (*((AX_U32 *)((AX_ADDR)pstSrcImg->au64VirAddr[0]) + j) >> 16) & 0x3;
    }
}
...
static AX_VOID SAMPLE_IVE_CCL_GetOutStat(const AX_IVE_MEM_INFO_T* pstSrcMem)
{
    ...
    memcpy(&stBlob, (AX_VOID *)((AX_ADDR)pstSrcMem->u64VirAddr), sizeof(AX_IVE_CCBLOB_T));
    for (AX_S32 i = 0; i < 6; i++) {
        if (i == 0) { //label_status
            for (AX_S32 k = 0, j = 0; j < stDumpStat[i].u32Size; k++, j++) {
                *((AX_U8 *)((AX_ADDR)stDumpStat[i].u64VirAddr) + j) = stBlob.astRegion[k].u8LabelStatus & 0x1;
            }
        } else if (i == 1) { //area
            for (AX_S32 k = 0, j = 0; j < stDumpStat[i].u32Size; k++, j+=3) {
                *((AX_U8 *)((AX_ADDR)stDumpStat[i].u64VirAddr) + j + 2) = (stBlob.astRegion[k].u32Area >> 16) & 0xF;
                *((AX_U8 *)((AX_ADDR)stDumpStat[i].u64VirAddr) + j + 1) = (stBlob.astRegion[k].u32Area >> 8) & 0xFF;
                *((AX_U8 *)((AX_ADDR)stDumpStat[i].u64VirAddr) + j) = (stBlob.astRegion[k].u32Area) & 0xFF;
            }
        } else if (i == 2) { //left
            for (AX_S32 k = 0, j = 0; j < stDumpStat[i].u32Size; k++, j+=2) {
                *((AX_U8 *)((AX_ADDR)stDumpStat[i].u64VirAddr) + j + 1) = (stBlob.astRegion[k].u16Left >> 8) & 0x7;
                *((AX_U8 *)((AX_ADDR)stDumpStat[i].u64VirAddr) + j) = (stBlob.astRegion[k].u16Left) & 0xFF;
            }
        } else if (i == 3) { //right
            for (AX_S32 k = 0, j = 0; j < stDumpStat[i].u32Size; k++, j+=2) {
                *((AX_U8 *)((AX_ADDR)stDumpStat[i].u64VirAddr) + j + 1) = (stBlob.astRegion[k].u16Right >> 8) & 0x7;
                *((AX_U8 *)((AX_ADDR)stDumpStat[i].u64VirAddr) + j) = (stBlob.astRegion[k].u16Right) & 0xFF;
            }
        } else if (i == 4) { //top
            for (AX_S32 k = 0, j = 0; j < stDumpStat[i].u32Size; k++, j+=2) {
                *((AX_U8 *)((AX_ADDR)stDumpStat[i].u64VirAddr) + j + 1) = (stBlob.astRegion[k].u16Top >> 8) & 0x3;
                *((AX_U8 *)((AX_ADDR)stDumpStat[i].u64VirAddr) + j) = (stBlob.astRegion[k].u16Top) & 0xFF;
            }
        } else if (i == 5) { //bottom
            for (AX_S32 k = 0, j = 0; j < stDumpStat[i].u32Size; k++, j+=2) {
                *((AX_U8 *)((AX_ADDR)stDumpStat[i].u64VirAddr) + j + 1) = (stBlob.astRegion[k].u16Bottom >> 8) & 0x3;
                *((AX_U8 *)((AX_ADDR)stDumpStat[i].u64VirAddr) + j) = (stBlob.astRegion[k].u16Bottom) & 0xFF;
            }
        }
    }
}
...
static AX_S32 SAMPLE_IVE_CCL_WriteFile(AX_IVE_IMAGE_T* pstImage, FILE* pFp)
{
    ...
    pU8 = (AX_U8 *)(AX_UL)((AX_ADDR)pstImage->au64VirAddr[0]);
...
static AX_S32 SAMPLE_IVE_TestCclProc(TEST_CCL_T* pstTestCcl, AX_CHAR *pchParamsList)
{
    ...
    u32BlobCnt = *((AX_U16 *)((AX_ADDR)pstTestCcl->stBlob.u64VirAddr));
